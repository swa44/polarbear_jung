...
import { sendFanOrderNotifyAlimtalk } from '@/lib/alimtalk'
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  try {
    ...
    if (error || !quote) return NextResponse.json({ error: '견적서를 찾을 수 없습니다.' }, { status: 404 })

    const now = Date.now()
    const expiresAtMs = quote.quote_expires_at ? new Date(quote.quote_expires_at).getTime() : 0
    if (quote.quote_expires_at && now > expiresAtMs && !['paid', 'shipped'].includes(quote.status)) {
      if (quote.status !== 'expired') {
        await supabase.from('fan_orders').update({ status: 'expired' }).eq('id', quote.id)
      }
      quote.status = 'expired'
    }

    return NextResponse.json({ quote, bank: getBankInfo() })
  } catch (e) {
    console.error(e)
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  try {
    const { token } = await params
    const { recipientName, receiverPhone, shippingAddress, shippingDetail, shippingMemo } =
      await req.json() as {
    ...
    const { data: quote, error: fetchError } = await supabase
      .from('fan_orders')
      .select('id, status, quote_expires_at, customer_phone')
      .eq('quote_token', token)
      .single()

    if (fetchError || !quote) return NextResponse.json({ error: '견적서를 찾을 수 없습니다.' }, { status: 404 })
    if (['cancelled', 'expired'].includes(quote.status))
      return NextResponse.json({ error: '종료된 견적서입니다.' }, { status: 400 })
    ...
    if (settingsData?.value) {
      ...
      sendFanOrderNotifyAlimtalk(phones).catch((e) => console.error('[Fan notify error]', e))
    }
